...
class FG_eval {
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    // The part of the state for the error of cte, psi and reference velocity
    ...
    // Minimize actuators action
    for(unsigned int t = 0; t < N-1; t++){
      fg[0] += weight_delta * CppAD::pow(vars[delta_start + t], 2);
      fg[0] += weight_a * CppAD::pow(vars[delta_start + t], 2);
    }

    // Minimize the value gap between sequential actuations
    for(unsigned int t = 0; t < N-2; t++){
      fg[0] += weight_delta_gap * CppAD::pow(vars[delta_start + t + 1] - vars[delta_start + t], 2);
      fg[0] += weight_a_gap * CppAD::pow(vars[a_start + t + 1] - vars[a_start + t], 2);
    }

    // Setup constraints
    // Initial constraints
    //
    // Add 1 to each of starting indices due to cost being located at fg[0]
